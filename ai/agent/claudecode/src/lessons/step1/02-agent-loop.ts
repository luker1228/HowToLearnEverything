...
type ToolMap = Record<ToolName, (input: { command: string }) => string>;
...
function toDeepSeekMessages(messages: Message[]): DeepSeekChatMessage[] {
  ...
  const converted: DeepSeekChatMessage[] = messages.map((message) => {
    ...
    if (message.role === "assistant") {
      ...
      const toolCalls = message.content
        .filter((block): block is AssistantToolUseBlock => block.type === "tool_use")
        .map((block) => ({
          id: block.id,
...
function parseAssistantMessage(message: DeepSeekChatCompletion["choices"][number]["message"]): ModelResponse {
...
async function callDeepSeekModel(messages: Message[]): Promise<ModelResponse> {
  loadLessonEnv();

  const apiKey = process.env.DEEPSEEK_API_KEY;
  if (!apiKey) {
    throw new Error(
      "Missing DEEPSEEK_API_KEY. Set it in src/lessons/.env or export it before running.",
    );
  }
  ...
  const completion = payload as DeepSeekChatCompletion;
...
function executeToolCalls(blocks: AssistantBlock[]): ToolResultMessage[] {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runLesson02().catch((error: unknown) => {
    console.error(error);
    process.exitCode = 1;
  });
}
