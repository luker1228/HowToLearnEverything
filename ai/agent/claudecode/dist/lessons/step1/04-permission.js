...
function isOutsideWorkspacePath(userPath) {
    const absolutePath = resolve(WORKDIR, userPath);
    const relativePath = relative(WORKDIR, absolutePath);
    return relativePath.startsWith("..") || relativePath.startsWith(`..${sep}`);
}
function run_read(path, limit = null) {
    try {
...
function run_write(path, content) {
    try {
...
function run_edit(path, old_text, new_text) {
    try {
        const filePath = safePath(path);
...
async function checkPermission(block) {
    if (block.name === "bash") {
    ...
    const ruleReason = checkRules(block.name, block.input);
    if (ruleReason) {
        const decision = await askUser(block.name, block.input, ruleReason);
        if (decision === "deny") {
            return false;
        }
    }
...
function globSearch(rootDir, pattern) {
    ...
    return matches.sort((left, right) => relative(rootDir, left).localeCompare(relative(rootDir, right)));
}
...
function executeToolCall(block, context) {
    switch (block.name) {
        case "bash":
            return {
                role: "tool",
...
async function executeToolCalls(blocks, context) {
    const results = [];
...
function getToolResult(messages, toolUseId) {
    return messages.find((message) => {
        return message.role === "tool" && message.toolUseId === toolUseId;
    });
}
function simulateModel(messages) {
