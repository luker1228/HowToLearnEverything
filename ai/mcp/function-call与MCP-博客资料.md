...
Function calling 从没真正「连上别的应用」。它只让模型能向**当前这个程序**发出一次结构化调用。连 GitHub / Slack 的，是运行时接到这次调用之后你手写的胶水。
...
Function calling 在这里是**当时唯一能用的原语**，不是为跨应用集成设计的方案。像 USB 出现前，什么外设都焊在主板上：能工作，只是每换一台电脑重焊一次。
...
> [!tip] 成稿可用的收束句
> 它们一直在解决不同问题；function calling 只是被临时拿来承担了 MCP 后来才拆出去的那一层。现在不该互相替代，该叠着用。
