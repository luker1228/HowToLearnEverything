...
html + css + js。刚上手别被代码量和语法吓到，那些不重要。重要的是模型：

- **html**：骨架，搭 DOM 结构。缺购买页、缺按钮，都是结构问题。
- **css**：装修，颜色、字号、布局怎么画出来。
- **js**：神经。点击、hover 这些事件发生时，你要跑什么逻辑。

这里面，我感觉最难理解的是布局。即**容器标签**的作用。

标签本身不难记：`div`、`section`、`form`、`label`、`input`……难的是：**你眼睛看到的一块 UI，背后套了几层盒子，每一层各自管什么。**

读布局不是一次把整棵 DOM 树背出来，而是**自上而下，一层一层展开**——跟读后端模块分层一个道理：先看见三层，再点开中间那层。

所以有一句我自己用的入门标准：

> 当你打开一个网页，能先一眼切出大区，再对某一区往下拆——而不是只看到「一个登录框」——布局就算入门了。
